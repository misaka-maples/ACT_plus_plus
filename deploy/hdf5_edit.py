...
def modify_hdf5(file_path, compress=None, truncate_ranges=None, edit=False):
    ...
    try:
        with h5py.File(file_path, 'r+') as f:
            ...
            # 添加新检查的路径
            additional_paths = ['gp/gpforce', 'gp/gppos', 'gp/gpstate']
            for path in additional_paths:
                if path not in f or f[path].size == 0:
                    raise KeyError(f"Path '{path}' not found or empty in the HDF5 file.")
                else:
                    print(f"Path '{path}' exists and has data.")
            ...
            # Ensure necessary paths exist and are not empty
            if not top_paths or not right_paths or not qpos_paths or not action_paths:
                raise KeyError(f"Required paths not found in the HDF5 file.")

            # Get the first matching path for each category
            top = top_paths[0] if top_paths else None
            right = right_paths[0] if right_paths else None
            qpos = qpos_paths[0] if qpos_paths else None
            action = action_paths[0] if action_paths else None
            left = left_paths[0] if left_paths else None

            # Check if paths are present and not empty
            if not all([top, right, qpos, action]):
                raise KeyError(f"One or more required paths are missing or empty in the HDF5 file.")
            
            # Check if each path exists in the file and is not empty
            for path in [top, right, qpos, action]:
                if path not in f or f[path].size == 0:
                    raise KeyError(f"Path '{path}' not found or empty in the HDF5 file.")
            
            if left and (left not in f or f[left].size == 0):
                raise KeyError(f"Path '{left}' not found or empty in the HDF5 file.")

            # Retrieve original data
            camera_top_data = f[top][:]
            camera_right_data = f[right][:]
            camera_left_data = f[left][:] if left else None
            qpos_data = f[qpos][:]
            actions = f[action][:]
            gpforce_data = f['gp/gpforce'][:]  # 获取 gpforce 数据
            gppos_data = f['gp/gppos'][:]  # 获取 gppos 数据
            gpstate_data = f['gp/gpstate'][:]  # 获取 gpstate 数据
            print(f"datalen:{len(actions)},{actions[0]},{[hex(i) for i in gppos_data]}")
            if edit:
                ...
                # Decompress camera data if not already in the expected shape
                if camera_right_data.shape[1:] != (480, 640, 3) and compress:
                    camera_right_data = decompress_images(camera_right_data)
                if camera_top_data.shape[1:] != (480, 640, 3) and compress:
                    camera_top_data = decompress_images(camera_top_data)
                if camera_left_data is not None and camera_left_data.shape[1:] != (480, 640, 3) and compress:
                    camera_left_data = decompress_images(camera_left_data)

                # Truncate data if truncation ranges are provided
                ...
                # if camera_left_data is not None:
                #     camera_left_data = truncate_data(camera_left_data, 'left_wrist')
                # qpos_data = truncate_data(qpos_data, 'qpos')
                # actions = truncate_data(actions, 'action')
                # gpforce_data = truncate_data(gpforce_data, 'gpforce')
                # gppos_data = truncate_data(gppos_data, 'gppos')
                # gpstate_data = truncate_data(gpstate_data, 'gpstate')
               
                # Create new paths and write data
                new_paths_top = ['observations/images/top']
                new_paths_right = ['observations/images/right_wrist']
                new_paths_left = ['observations/images/left_wrist'] if camera_left_data is not None else []
                new_qpos_path = ['observations/qpos']
                new_actions_path = ['action']
                new_gpforce_path = ['gp/gpforce']  # 新路径 gp/gpforce
                new_gppos_path = ['gp/gppos']  # 新路径 gp/gppos
                new_gpstate_path = ['gp/gpstate']  # 新路径 gp/gpstate
                ...
                for path in new_qpos_path:
                    if path in f:
                        del f[path]
                    f.create_dataset(path, data=qpos_data)
                ...
                # 写入新的路径数据
                for path in new_gpforce_path:
                    if path in f:
                        del f[path]
                    f.create_dataset(path, data=gpforce_data)

                for path in new_gppos_path:
                    if path in f:
                        del f[path]
                    f.create_dataset(path, data=gppos_data)

                for path in new_gpstate_path:
                    if path in f:
                        del f[path]
                    f.create_dataset(path, data=gpstate_data)
    ...
    try:
        with h5py.File(dataset_path, 'r') as f:
            ...
            if right_image_path not in f:
                print(f"Path '{right_image_path}' not found in the HDF5 file.")
                right_image_path=None
            top_image_path = 'observations/images/top'
            if top_image_path not in f:
                print(f"Path '{top_image_path}' not found in the HDF5 file.")
                top_image_path=None
            left_image_path='observations/images/left_wrist'
            if left_image_path not in f:
                print(f"Path '{left_image_path}' not found in the HDF5 file.")
            ...
            if compressed and arm_data is not None:
...
if __name__ == '__main__':
    ...
    #     'qpos': (45, 100),
    # }
    modify_hdf5('/home/wfx/wfx-project/test/ACT_plus_plus-deploy_get_data/gp_episode_2_25_15/episode_1.hdf5', compress=False,edit=False)
    # batch_modify_hdf5(dataset_dir, output_dir, skip_mirrored_data=True)
    # 保存视频
    # for i in range(32,53):
    # save_video(r'/home/wfx/wfx-project/test/ACT_plus_plus-deploy_get_data/demo_data_trush', fps=20, i=0,arm='right_wrist')
    #
