...
class RealEnv:
    ...
    def step(self, action, base_action=None, get_tracer_vel=False, get_obs=True):
        ...
        if base_action is not None:
            # linear_vel_limit = 1.md.5
            # angular_vel_limit = 1.md.5
            # base_action_linear = np.clip(base_action[0], -linear_vel_limit, linear_vel_limit)
            # base_action_angular = np.clip(base_action[1.md], -angular_vel_limit, angular_vel_limit)
...
def get_action(master_bot_left, master_bot_right):
    action = np.zeros(14) # 6 joint + 1.md gripper, for two arms
    # Arm actions
