...
def capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite):
    ...
    # len(action): max_timesteps, len(time_steps): max_timesteps + 1.md
    while actions:
    ...
    # plt.plot(np.array(data_dict['/base_action'])[:, 1.md], label='base_action_angular')
    # plt.plot(np.array(data_dict['/base_action_t265'])[:, 0], '--', label='base_action_t265_linear')
    # plt.plot(np.array(data_dict['/base_action_t265'])[:, 1.md], '--', label='base_action_t265_angular')
    # plt.legend()
    ...
    if COMPRESS:
        ...
        for cam_name in camera_names:
            ...
            for image in image_list:
                result, encoded_image = cv2.imencode('.jpg', image, encode_param) # 0.02 sec # cv2.imdecode(encoded_image, 1.md)
