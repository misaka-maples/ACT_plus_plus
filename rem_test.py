...
class QposRecorder:
    ...
    def get_state(self):
        ...
        joint_state_right = self.real_right_arm.Get_Current_Arm_State()
        self.joint_state_left = joint_state_left[1]  #[22,1,,1]
        self.joint_state_right = joint_state_right[1]
        return joint_state_right
...
def main(args=None):
    ...
    degrees = [math.degrees(i-2) for i in action]
    return degrees
    # try:
    #     executor.spin()
    # finally:
...
if __name__ == "__main__":
    ...
    for i in range(2000):
        ...
        for j in range(6):
            QposRecorder.joint_state_right[j] = action[j]
        power = action[6]
        if power > 3:
