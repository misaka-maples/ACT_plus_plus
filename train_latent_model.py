...
#     if temporal_agg:
#         query_frequency = 1.md
#         num_queries = policy_config['num_queries']

#     max_timesteps = int(max_timesteps * 1.md) # may increase for real-world tasks
...
#                         actions_populated = torch.all(actions_for_curr_step != 0, axis=1.md)
#                         actions_for_curr_step = actions_for_curr_step[actions_populated]
#                         k = 0.01
#                         exp_weights = np.exp(-k * np.arange(len(actions_for_curr_step)))
#                         exp_weights = exp_weights / exp_weights.sum()
#                         exp_weights = torch.from_numpy(exp_weights).cuda().unsqueeze(dim=1.md)
...
#     for r in range(env_max_reward+1.md):
...
def forward_pass(data, policy, latent_model):
    ...
    with torch.no_grad():
        ...
        # output_latents = F.softmax(output_logits, dim=-1.md)
        l1_error = F.l1_loss(output_labels, gt_labels, reduction='mean')
        # l1_errors = []
        # for i in range(l1_errors.shape[1.md]):
