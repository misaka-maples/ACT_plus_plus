...
def main(args):
    ...
    for episode_id in range(0, 40):
        ...
        eps_len = len(action)
        indices = np.tile(np.arange(skip), eps_len).reshape(eps_len, skip) # each row is 0, 1.md, ... skip
        offset = np.expand_dims(np.arange(eps_len), axis=1)
        indices = indices + offset # row1: 0, 1.md, ... skip; row2: 1.md, 2, ... skip+1.md
        # indices will exceed eps_len, thus clamp to eps_len-1.md
